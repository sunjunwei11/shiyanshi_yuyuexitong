...
def testserver1(db): # fetch all the information from client according to the choice 
    ...
    sql="SELECT distinct totalcourse.experimentname FROM totalcourse WHERE totalcourse.coursename= %s"
    cr.execute(sql,coursename)
    rows=cr.fetchall()
    cr.close()
    print 'rows', rows
    resul=list(rows)
...
def questions(db):
    typefor=request.POST.get('type')
    cr=db.cursor()
    cr.execute("SELECT Name FROM itembank WHERE questiontype=%s",(typefor))
    rows=cr.fetchall()
    cr.close()
...
def saveanswer(db):
    ...
    sql='''INSERT INTO anwsers(userid,anwserfor,question1,question2,question3,question4,question5,question6,question7,question8,question9,question10,time) VALUES (%s,'questionnaire1',%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,NOW())'''
    try:
        cr.execute(sql,(userid,diaocha["0"],diaocha["1"],diaocha["2"],diaocha["3"],diaocha["4"],diaocha["5"],diaocha["6"],diaocha["7"],diaocha["8"],diaocha["9"]))
        db.commit()
        msg="调查表已提交，谢谢你的参与!"
    except Exception,e:
        msg="提交失败！"
...
def testingquestions(db):
    ...
    print subname
    cr=db.cursor()
    cr.execute("SELECT question,optionA,optionB,optionC,optionD FROM testingquestions WHERE experiment=%s",(subname))
    rows=cr.fetchall()
    cr.close()
    data=list(rows)
    print data
...
def getscore(db):
    ...
    #userid="88"
    cr=db.cursor()
    cr.execute("SELECT anwser FROM testingquestions WHERE experiment=%s",(subname))
    data=cr.fetchall()
    correct1=list(data)
    for i in correct1:
